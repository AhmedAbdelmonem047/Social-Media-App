...
exports.listFiles = exports.deleteFiles = exports.deleteFile = exports.createGetFilePresignedURL = exports.getFile = exports.createUploadFilePresignedURL = exports.uploadFiles = exports.uploadLargeFile = exports.uploadFile = exports.s3Client = void 0;
...
const uploadFile = async ({ storeType = multer_cloud_1.StorageType.cloud, Bucket = process.env.AWS_BUCKET_NAME, path = "general", ACL = "private", file }) => {
    const command = new client_s3_1.PutObjectCommand({
        Bucket,
        ACL,
        Key: `${process.env.APPLICATION_NAME}/${path}/${(0, uuid_1.v4)()}_${file.originalname}}`,
        Body: storeType === multer_cloud_1.StorageType.cloud ? file.buffer : (0, fs_1.createReadStream)(file.path),
...
const uploadLargeFile = async ({ storeType = multer_cloud_1.StorageType.cloud, Bucket = process.env.AWS_BUCKET_NAME, path = "general", ACL = "private", file }) => {
...
const uploadFiles = async ({ storeType = multer_cloud_1.StorageType.cloud, Bucket = process.env.AWS_BUCKET_NAME, path = "general", ACL = "private", files, useLarge = false }) => {
    let urls = [];
    if (useLarge)
...
const createUploadFilePresignedURL = async ({ Bucket = process.env.AWS_BUCKET_NAME, path = "general", originalname, ContentType, expiresIn = 60 * 60 }) => {
    const Key = `${process.env.APPLICATION_NAME}/${path}/${(0, uuid_1.v4)()}_presigned_${originalname}}`;
    const command = new client_s3_1.PutObjectCommand({
        Bucket,
        Key,
        ContentType
